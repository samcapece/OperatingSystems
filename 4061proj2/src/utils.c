...
void perfectFormat(char *buffer, const char *find, const char *replace) {
    ...
    while ((occurrence = strstr(occurrence, find))) {
        memmove(occurrence + replace_len, occurrence + find_len, strlen(occurrence + find_len) + 1);
        memcpy(occurrence, replace, replace_len);
        occurrence += replace_len;
    }
}
